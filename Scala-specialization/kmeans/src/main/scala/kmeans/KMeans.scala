...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    var mappedPoints = points.groupBy(findClosest(_, means))
    val empty = means.filterNot(mappedPoints.contains)
    mappedPoints = empty.foldLeft(mappedPoints) { (map, mean) => if (!map.contains(mean)) map +(mean, GenSeq[Point]()) else map }
    mappedPoints
  }
  ...
  def update(classified: GenMap[Point, GenSeq[Point]], oldMeans: GenSeq[Point]): GenSeq[Point] = {
    val updated = oldMeans.map(old => findAverage(old, classified(old)))
    updated
  }

  def converged(eta: Double)(oldMeans: GenSeq[Point], newMeans: GenSeq[Point]): Boolean = {
    def squareDistance(f: Point, s: Point): Double = Math.pow(f.x - s.x, 2) + Math.pow(f.y - s.y, 2) + Math.pow(f.z - s.z, 2)
    var i = 0
    while (i < oldMeans.size) {
      val distance = squareDistance(oldMeans(i), newMeans(i))
      if (distance > eta) return false
      i += 1
    }
