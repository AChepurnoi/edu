...
object TweetLength {
  ...
  def colorForRemainingCharsCount(remainingCharsCount: Signal[Int]): Signal[String] = {
    def getColor(remainingCharsCount: Signal[Int]): String = {
      if (remainingCharsCount() >= 15) return "green"
      if (remainingCharsCount() < 0) return "red"
      return "orange"
    }
    Signal(getColor(remainingCharsCount))
  }
